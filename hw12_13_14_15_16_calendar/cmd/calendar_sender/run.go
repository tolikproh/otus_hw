...
func run() {
	...
	eventSender := app.NewEventSender(calendar, log, broker)
	go eventSender.Start(ctx)

	log.Info("calendar_sender service is running...")

	<-ctx.Done()

	log.Info("calendar_sender service was stopped")
}
